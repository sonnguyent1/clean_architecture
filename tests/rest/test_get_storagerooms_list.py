...
from clean_architecture.domain.models import StorageRoom
...
@mock.patch('clean_architecture.use_cases.storageroom_use_case.StorageRoomListUseCase')
...
def test_get_failed_response(mock_use_case, client):
    ...
    assert json.loads(http_response.data.decode('UTF-8')) == {'type': res.ResponseFailure.SYSTEM_ERROR,
...
def test_request_object_initialisation_and_use_with_filters(mock_use_case, client):
    ...
    with mock.patch('clean_architecture.use_cases.request_objects.StorageRoomListRequestObject') as mock_class_req_obj:
        mock_class_req_obj.from_dict.return_value = internal_request_object
        client.get('/storagerooms?filter_param1=value1&filter_param2=value2')

        mock_class_req_obj.from_dict.assert_called_with(
